...
def main():
    ...
    argument_spec.update(
        ...
        earlier_epoch_id=dict(type='str', aliases=[
                              'earlier_epoch_uuid', 'earlier_epoch']),
        later_epoch_id=dict(type='str', aliases=[
                            'later_epoch_uuid', 'later_epoch']),
    ...
    module = AnsibleModule(
        ...
        required_if=[['state', 'validate', ['name']],
                     ...
                     ['state', 'present', ('earlier_epoch_id',
                                           'earlier_epoch_time'), True],
                     ['state', 'present', ('later_epoch_id', 'later_epoch_time'), True]],
        mutually_exclusive=[('earlier_epoch_id', 'earlier_epoch_time'),
                            ('later_epoch_id', 'later_epoch_time')],
        required_together=[('earlier_epoch_id', 'later_epoch_id'),
                           ('earlier_epoch_time', 'later_epoch_time')]
    )
    ...
    if state == 'query':
        if name is None:
            delta_job_list = ndi.query_delta_analysis(
        ...
        if earlier_epoch_id and later_epoch_id:
            data = {
                "jobName": name,
                "priorEpochUuid": earlier_epoch_id,
                "laterEpochUuid": later_epoch_id
            }
        elif earlier_epoch_time and later_epoch_time:
            earlier_epoch_dt = datetime.datetime.fromisoformat(
                earlier_epoch_time.replace('Z', ''))
            later_epoch_dt = datetime.datetime.fromisoformat(
                later_epoch_time.replace('Z', ''))
            data = {
                "jobName": name,
        ...
        trigger_path = ndi.config_ig_path + "/" + \
            ndi.run_epoch_delta_ig_path.format(insights_group, site_name)
        resp = nd.request(trigger_path, method='POST',
                          data=data, prefix=ndi.prefix)

        if resp["success"] is True:
            job_id = resp["value"]["data"]["configId"]
            delta_job_info = ndi.query_delta_analysis(
                insights_group, site_name, jobId=job_id)
        ...
        if nd.existing and job_id:
            if module.check_mode:
                ...
                rm_path = ndi.config_ig_path + "/" + \
                    "{0}/fabric/{1}/deleteEpochDelta".format(
                        insights_group, site_name)
                rm_payload = [job_id]

                rm_resp = nd.request(rm_path, method='POST',
                                     data=rm_payload, prefix=ndi.prefix)

                if rm_resp["success"] is True:
                    nd.existing = {}
                else:
                    nd.fail_json(
                        msg="Delta Analysis {0} could not be deleted".format(name))
