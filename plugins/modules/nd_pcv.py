...
options:
  ...
  state:
    description:
    - Use C(present) or C(absent) for creating or deleting pre-change validation job.
    - Use C(query) for listing existing pre-change validation job.
    - Use C(wait_and_query) to execute the query until pre-change verification task status value is COMPLETED or FAILED
    type: str
...
def main():
    ...
    argument_spec.update(
        ...
        state=dict(type='str', default='query', choices=[
                   'query', 'absent', 'present', 'wait_and_query']),
    )
    ...
    if state == 'wait_and_query' and nd.existing:
        ...
        if nd.existing and job_id:
            if module.check_mode:
                ...
                rm_path = '{0}/{1}/prechangeAnalysis/jobs'.format(
                    path, insights_group)
                rm_payload = [job_id]
                ...
                if rm_resp["success"] is True:
                    ...
                    nd.fail_json(
                        msg="Pre-change validation {0} is not able to be deleted".format(name))
        ...
        if file:
            ...
            create_pcv_path = '{0}/{1}/fabric/{2}/prechangeAnalysis/fileChanges'.format(
                path, insights_group, site_name)
            file_resp = nd.request(create_pcv_path, method='POST', file=os.path.abspath(
                file), data=data, prefix=ndi.prefix)
            if file_resp.get("success") is True:
                nd.existing = file_resp.get("value")["data"]
        elif manual:
            data["imdata"] = json.loads(manual)
            create_pcv_path = '{0}/{1}/fabric/{2}/prechangeAnalysis/manualChanges?action=RUN'.format(
                path, insights_group, site_name)
            manual_resp = nd.request(
                create_pcv_path, method='POST', data=data, prefix=ndi.prefix)
            if manual_resp.get("success") is True:
                nd.existing = manual_resp.get("value")["data"]
        else:
            nd.fail_json(
                msg="either file or manual is required to create pcv job")
    nd.exit_json()
