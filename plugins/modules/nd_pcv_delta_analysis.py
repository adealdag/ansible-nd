...
options:
  ...
  epoch_choice:
    description:
    - Epoch choice when querying anomalies.
    - Use epoch2 to choose anomalies only for epoch2.
    - Use epoch1 to choose anomalies only for epoch1.
    ...
    if state == "validate" and anomalies:
        ...
        for anomaly in anomalies:
            ...
            if severity == "critical":
                critical += 1
        nd.fail_json(msg="Pre-change analysis failed. The above {0}(critical({1})|major({2})|minor({3})|warning({4})) anomalies have been detected.".format(len(anomalies), critical, major, minor, warning))
