...
class NDI:
    ...
    def query_compliance_score(self, ig_name, site_name, compliance_epoch_id):
        ig_base_path = self.event_insight_group_path.format(ig_name, site_name)
        path = "{0}/{1}/complianceScore?%24epochId={2}".format(
            ig_base_path, self.compliance_path, compliance_epoch_id)
        return self.query_data(path)

    def query_compliance_count(self, ig_name, site_name, compliance_epoch_id):
        ig_base_path = self.event_insight_group_path.format(ig_name, site_name)
        path = "{0}/{1}/count?%24epochId={2}".format(
            ig_base_path, self.compliance_path, compliance_epoch_id)
        return self.query_data(path)
    ...
    def query_entry(self, path, size):
        ...
        if entries is None or len(entries) == 0:
            ...
            for page in range(1, pages):
                obj = self.nd.query_obj(path.format(
                    size, page), prefix=self.prefix)
                entries += obj.get('entries')
            return entries

    def query_anomalies(self, ig_name, site_name, epoch_delta_job_id, epoch_choice, exclude_ack_anomalies):
        epoch_delta_ig_path = self.epoch_delta_ig_path.format(
            ig_name, site_name, epoch_delta_job_id)
        size = 100
    ...
    def query_impacted_resource(self, ig_name, site_name, epoch_delta_job_id):
        ...
        formated_impacted_resource = self.format_impacted_resource(
            impacted_resource)
    ...
    def query_compliance_smart_event(self, ig_name, site_name, compliance_epoch_id):
        ig_base_path = self.event_insight_group_path.format(ig_name, site_name)
        size = 100
        path = "{0}/smartEvents?%24epochId={1}".format(
            ig_base_path, compliance_epoch_id)
        event_path = path + \
            "&%24size={0}&%24page={1}&%24sort=-severity&category=COMPLIANCE"
    ...
    def query_unhealthy_resources(self, ig_name, site_name, compliance_epoch_id):
        result = {}
        ig_base_path = self.event_insight_group_path.format(ig_name, site_name)
        path = "{0}/{1}/eventUnhealthyResources?%24epochId={2}".format(
            ig_base_path, self.compliance_path, compliance_epoch_id)
        objs = self.query_data(path)
        for obj in objs:
            result[obj.get("bucket")] = {"count": obj.get(
                "count"), "total": obj.get("total")}
        return result

    def query_pcvs(self, ig_name):
        pcvs_path = '{0}/{1}/prechangeAnalysis?$sort=-analysisSubmissionTime'.format(
            self.config_ig_path, ig_name)
    ...
    def query_pcv(self, ig_name, site_name, pcv_name):
        ...
        if pcv_name is not None and site_name is not None:
            ...
            pcv_path = '{0}/{1}/fabric/{2}/prechangeAnalysis'.format(
                self.config_ig_path, ig_name, site_name)
            pcv_result = self.get_pre_change_result(
                pcv_results, pcv_name, site_id, pcv_path, prefix=self.prefix)
        else:
            self.nd.fail_json(
                msg="site name and prechange validation job name are required")
        return pcv_result
    ...
    def copy_children(self, tree):
        ...
        for dn, children in cmap.items():
            ...
            json_path_expr_search = parse(
                '$..children.[*].{0}'.format(aci_class))
